...
public class BinaryIndexedTree {
	...
	public static int getSum(int index) {
		...
		while (index > 0) {// if index=15=binary=1111=positions (MSB)8,(8+4)=12,(12+2)=14,(14+1)=15 indices
			value += bit[index];
			index -= (index & -index);// -index=2's complement of index; index&-index = largest power of 2 that divides index perfectly; index = index-(index&-index) = subtracting = getting parent of index;subtraction = difference will be power of 2
										// = go to parent by decrementing
		}
		return value;
	}
}
